#!/usr/bin/env bash
# Phase 1 iteration 2: student-noisy-v1 becomes the teacher.
# Re-pseudo-labels the extracted 286k unlabeled pool at a stricter
# --min-conf 0.8, then trains student-noisy-v2 with the same opt3 recipe.
...
PSEUDO_DIR=data/pseudo_labels_v2

echo "=== $(date -Is) [1/2] pseudo-labels v2 (teacher: student-noisy-v1, min-conf 0.8) ==="
python src/generate_pseudo_labels.py \
    ...
    > "$LOGS/phase1-pseudo-v2.log" 2>&1

echo "=== $(date -Is) [2/2] training student-noisy-v2 (opt3 recipe + pseudo v2) ==="
python src/patch_classification.py \
    ...
    --output-dir "$DL" \
    > "$LOGS/student-noisy-v2.log" 2>&1

echo "=== $(date -Is) iteration 2 complete ==="
